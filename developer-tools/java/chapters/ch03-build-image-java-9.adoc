...
WORKDIR opt
RUN tar -xf /opt/openjdk-9_linux-x64_bin.tar.gz
...
running Docker container with JDK 9 installed. replace $PWD with your current directory i.e. helloworld-java-9 directory full path:
